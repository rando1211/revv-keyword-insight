...
serve(async (req) => {
  ...
  try {
    ...
    // First, let's try to create a website/page using the correct GHL API
    // Using the websites API instead of funnels API
    const ghlResponse = await fetch(`https://services.leadconnectorhq.com/websites/`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        domainName: '', // Will be assigned by GHL
        html: htmlContent,
    ...
    if (!ghlResponse.ok) {
      ...
      // If websites API doesn't work, try the pages API as fallback
      console.log('Trying pages API as fallback...');
      const pagesResponse = await fetch(`https://services.leadconnectorhq.com/pages/`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${ghlApiKey}`,
          'Content-Type': 'application/json',
